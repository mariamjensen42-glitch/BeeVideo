# BeeVideo 长期记忆
> 只留**仍生效的硬约束**；论证见 `YYYY-MM-DD.md`，细节见 `REFERENCE.md`。超 3000 字符会被截断注入。
...
## 发布（release）
- 签名 `keys/beevideo-release.jks`（alias `beevideo`），口令在 `local.properties` 的 `beevideo.keystore.*`；均已 gitignore。
- R8 开着。`app/proguard-rules.pro` 里 **`-keep class com.github.catvod.** { *; }` 与 `-dontobfuscate` 都不能删**：
  前者防**删除**（那些方法只给外部 jar 用，R8 眼里是死代码），后者防**改名**（jar 字节码写死了宿主类名）。
- 产物 5.16 MB（debug 75.5 MB）；资源名会被改成 `res/E4.xml` 这种，**是正常的**。
- 改完构建配置后跑 `.workbuddy/scripts/verify_release.py <release.apk> <debug.apk>`，差集必须为空。
- ⚠️ release 与 debug 签名不同 → 换装必须先卸载，**App 内已配的源会全丢**。

## CatVod 兼容层 —— 签名 = ABI，改成员前先跑 `dex_probe.py`
⚠️ jar **预编译**：少一个成员 / 静态写成实例 → 运行时 `NoSuchMethodError`，**编译期零提示**。
判定依据是 `method_ids` 里的 `invoke-*` opcode（真值表见 REFERENCE.md）。
...
## 组件
外壳只用 `Scaffold`；已删 `BeePage`/`BeeTopBar`/`SectionTitle`/`BeeEmptyState`。自建只剩 `PosterCard`/`ScoreBadge`、
`BeeChipRow<T>`（**不用 `FilterChip`**）、`HeroCarousel`（**槽位优先**）。**默认值即规范值就不传**。
...
- ⚠️ **adb server 每次调用都被回收** → 要 App 联网必须在**同一次调用里**先建 `adb reverse`。
- ⚠️ uiautomator dump 用 `ui_text.py`（ElementTree），**别写正则**（值含 `"` 时改单引号）。
- `TV-Multiplatform-main` / `FongMi/TV` 都是 **GPL-3.0**：只作行为规格，**抄源码 = 整体开源**。

## 现状与待办
已验：type=1/0/3、HTTPS 播放、JS/Py 显式拒绝、ext 四种形态、jar ABI、**release 包已出**。
待办：**本地代理服务**、搜索入口、主题开关；仓库**尚无任何 commit**。
